    ...
    public class MainDialog : IDialog<BasicForm>
    {
        ...
        private async Task FormComplete(IDialogContext context, IAwaitable<BasicForm> result)
        {
            try
            {
                ...
                if (form != null)
                {
                    await context.PostAsync("Thanks for completing the form! Just type anything to file a new incident.");
                    MessagesController.LastImage = null;
                    MessagesController.LastImageName = String.Empty;
                    MessagesController.LastImageType = String.Empty;
                }
                else
                {
            ...
            {
                await context.PostAsync("You canceled the form! Type anything to file a new incident.");
            }
